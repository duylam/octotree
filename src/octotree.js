(() => {
  const waitForStorageInit = new Promise((resolve) => {
    // Init the storage for supporting plugins which run before document's "ready" event
    parallel(
      Object.keys(STORE),
      (key, cb) => BROWSER_STORAGE.setIfNull(STORE[key], DEFAULTS[key], cb),
      resolve
    );
  });
  ...
  Promise.all([waitForStorageInit, waitForDocumentReady]).then(loadExtension);
